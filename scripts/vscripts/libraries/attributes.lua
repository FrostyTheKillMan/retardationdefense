...
function Attributes:Init()
    ...
    local DEFAULT_HP_REGEN_PER_STR = 0.01
	local DEFAULT_MAGIC_RES_PER_STR = 0.08
	local DEFAULT_ARMOR_PER_AGI = 0.16
    local DEFAULT_ATKSPD_PER_AGI = 1
	local DEFAULT_MOVESPEED_PER_AGI = 0.05
	...
	Attributes.magic_res_adjustment = v.MAGIC_RES_PER_STR - DEFAULT_MAGIC_RES_PER_STR
	...
	Attributes.movespeed_adjustment = v.MOVESPEED_PER_AGI - DEFAULT_MOVESPEED_PER_AGI
    ...
    Timers:CreateTimer(function()
        ...
        if strength ~= hero.strength then
			...
			if not hero:HasModifier("modifier_magic_resistance_bonus") then
                Attributes.applier:ApplyDataDrivenModifier(hero, hero, "modifier_magic_resistance_bonus", {})
            end

            local magic_res_stacks = math.abs(strength * Attributes.magic_res_adjustment * 100)
            hero:SetModifierStackCount("modifier_magic_resistance_bonus", Attributes.applier, magic_res_stacks)
        end
        ...
        if agility ~= hero.agility then        
			...
			if not hero:HasModifier("modifier_movespeed_bonus") then
                Attributes.applier:ApplyDataDrivenModifier(hero, hero, "modifier_movespeed_bonus", {})
            end

            local movespeed_stacks = math.abs(agility * Attributes.movespeed_adjustment * 100)
            hero:SetModifierStackCount("modifier_movespeed_bonus", Attributes.applier, movespeed_stacks)
